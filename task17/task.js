/* 数据格式演示
var aqiSourceData = {
  "北京": {
    "2016-01-01": 10,
    "2016-01-02": 10,
    "2016-01-03": 10,
    "2016-01-04": 10
  }
};
*/
//跨浏览器事件监听
function addEventHandler(element, event, hanlder) {
    if (element.addEventListener) {
        element.addEventListener(event, hanlder, false);
    } else if (element.attachEvent) {
        element.attachEvent("on"+event, hanlder);
    } else  {
        element["on" + event] = hanlder;
    }
}
// 以下两个函数用于随机模拟生成测试数据
function getDateStr(dat) {
  var y = dat.getFullYear();
  var m = dat.getMonth() + 1;
  m = m < 10 ? '0' + m : m;
  var d = dat.getDate();
  d = d < 10 ? '0' + d : d;
  return y + '-' + m + '-' + d;
}
function randomBuildData(seed) {
  var returnData = {};
  var dat = new Date("2016-01-01");
  var datStr = ''
  for (var i = 1; i < 92; i++) {
    datStr = getDateStr(dat);
    returnData[datStr] = Math.ceil(Math.random() * seed);
    dat.setDate(dat.getDate() + 1);
  }
  return returnData;
}

var aqiSourceData = {
  "北京": randomBuildData(500),
  "上海": randomBuildData(300),
  "广州": randomBuildData(200),
  "深圳": randomBuildData(100),
  "成都": randomBuildData(300),
  "西安": randomBuildData(500),
  "福州": randomBuildData(100),
  "厦门": randomBuildData(100),
  "沈阳": randomBuildData(500)
};

// 用于渲染图表的数据
var chartData = {};

// 记录当前页面的表单选项
var pageState = {
  nowSelectCity: "北京",
  nowGraTime: "day"
}
// 获取需要用到的元素
var formGraTime = document.getElementById("form-gra-time");
var citySelect = document.getElementById("city-select");
var aqiChartWrap = document.getElementById("aqi-chart-wrap");
/**
 * 渲染图表
 */
function renderChart() {
  var color = "", text = "";
  // var divs = aqiChartWrap.getElementsByTagName("div");
  // aqiChartWrap.remove(divs);
  for (str in chartData) {
    color = '#' + Math.floor(Math.random() * 0xFFFFFF).toString(16);
    text += '<div title="'+str+":"+chartData[str]+'" style="height:'+chartData[str]+'px; background-color:'+color+'"></div>';
  }
  aqiChartWrap.innerHTML = text;
}

/**
 * 日、周、月的radio事件点击时的处理函数
 */
function graTimeChange() {
  // 确定是否选项发生了变化 
  if (pageState.nowGraTime === this.value) {
    return;
  } else {
    pageState.nowGraTime = this.value;
  }
  // 设置对应数据
  initAqiChartData();
  // 调用图表渲染函数
  renderChart();
}

/**
 * select发生变化时的处理函数
 */
function citySelectChange() {
  // 确定是否选项发生了变化 
  if (pageState.nowSelectCity === this.value) {
    return;
  } else {
    pageState.nowSelectCity = this.value;
  }
  // 设置对应数据
  initAqiChartData();
  // 调用图表渲染函数
  renderChart();
}

/**
 * 初始化日、周、月的radio事件，当点击时，调用函数graTimeChange
 */
function initGraTimeForm() {
  var inputs = formGraTime.getElementsByTagName("input");
  for (var i = 0; i < inputs.length; i++) {
    addEventHandler(inputs[i],'click',graTimeChange);
  };
}

/**
 * 初始化城市Select下拉选择框中的选项
 */
function initCitySelector() {
  // 读取aqiSourceData中的城市，然后设置id为city-select的下拉列表中的选项
  var text = "";
  for ( str in aqiSourceData) {
    text += '<option>' + str + '</option>'; 
    citySelect.innerHTML = text;
  }
  // 给select设置事件，当选项发生变化时调用函数citySelectChange
  addEventHandler(citySelect,'change',citySelectChange);
}

/**
 * 初始化图表需要的数据格式
 */
function initAqiChartData() {
  // 将原始的源数据处理成图表需要的数据格式
  var nowCityData = aqiSourceData[pageState.nowSelectCity];
  // 处理好的数据存到 chartData 中
  // 按天处理
  if (pageState.nowGraTime === "day") {
    chartData = nowCityData;
  }
  // 按周处理
  if (pageState.nowGraTime === "week"){
    chartData = {};
    var dayNum = 0, weekNum = -1, flag = false, theDay = 0, countNum = 0;
    for (riqi in nowCityData) {
      // 第一次进入循环时，用theDay变量存储所给日期中第一天的星期数
      if (!flag) {
        // 2016.1.1为周六theDay存为周五
        theDay = (new Date(riqi)).getDay();
        flag = true;
      }
      dayNum++;
      countNum += nowCityData[riqi];
      if ((new Date(riqi)).getDay() === theDay) {
        // 当遇到下一个周五时，表明数据已经统计了一周
        weekNum++;
        // 这里的加1是为了保证和人平时读数的习惯相符
        chartData['第'+(weekNum+1)+'周'] = Math.floor(countNum/dayNum);
        dayNum = 0;
        countNum = 0;
      }
    }
    // 余下的天数不足一周时，累计为一周
    if (dayNum !== 0) {
      weekNum++;
      chartData['第'+(weekNum+1)+'周'] = Math.floor(countNum/dayNum);
    }
  }
  // 按月处理
  if (pageState.nowGraTime === 'month') {
    chartData = {};
    var countNum=0, dayNum=0, monthNum=0;
    for (var item in nowCityData) {
      countNum += nowCityData[item];
      dayNum ++;
      if ((new Date(item)).getMonth() !== monthNum) {
        monthNum ++;
        chartData['第'+monthNum+'月'] = Math.floor(countNum/dayNum);
        countNum = 0
        dayNum = 0;
      }
    }
    if (dayNum != 0) {
      monthNum ++;
      chartData['第'+monthNum+'月'] = Math.floor(countNum/dayNum);
    }
  }
}

/**
 * 初始化函数
 */
function init() {
  initGraTimeForm()
  initCitySelector();
  initAqiChartData();
  renderChart();
}

init();